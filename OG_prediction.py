...
oilpricedata=oilpricedata.drop([0,1]) # remove first 2 rows
#oilpricedata["Date"]=oilpricedata["Date"].astype('datetime64[ns]') # Convert column to date format
...
oilpricedata[['date','oil_price']].set_index('date').plot(xlabel="Date",ylabel="Oil Price",color="green",title="Historical Oil Prices",linewidth=1.0)

# To begin with a more advanced data analysis, we will create a pairplot using seaborn to analyze BP share price.

## 2.2.- Pairplot on BP share price from years 2000 to 2017 using a color gradient for different years
#==============================================================================
# Pairplot using master data table (master_df) with a filter on BP share price
#==============================================================================

palette=sns.cubehelix_palette(18, start=2, rot=0, dark=0, light=.95, reverse=False)
sns.pairplot(master_df[master_df['name']=="BP.L"].drop(["share_price_scaled"],axis=1),
...
master_df13=master_df[master_df["year"]>2012]
